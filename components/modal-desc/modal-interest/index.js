						...
						>
							<Dialog.Panel className="w-full max-w-lg p-6 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
								<Dialog.Title as="h3" className="flex flex-row items-center justify-center leading-6 text-gray-900">
									<Image width={36} height={36} src={"/modal-desc/interest-modal.svg"} alt="" />
									<p className="ml-2 text-xl font-semibold text-secondary">Interests</p>
								</Dialog.Title>
								<div className="grid grid-cols-3 gap-2 mt-8 ">
									{props?.data?.map((value, index) => (
										<span className="self-center px-3 py-2 text-xs font-medium text-center text-white rounded bg-secondary" key={index}>
											{value}
										</span>
									))}
								</div>
