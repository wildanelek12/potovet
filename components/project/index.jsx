    ...
    <div className="flex flex-col">
      <Link href={`/project/${data.slug}`}>
        <div className="relative w-full overflow-hidden rounded-t-lg max-h-60 h-60">
          <Image src={`/${data.image_path}`} alt={data.title} className="absolute object-cover w-fit h-fit" fill />
        </div>
      ...
      <div className="flex flex-row items-center mt-2">
        <div className="flex flex-row items-center basis-2/3">
          <Link href={"/project"} className="flex flex-row items-center">
            <Image
              src={data?.user?.photo ? `/${data?.user?.photo}` : "https://st3.depositphotos.com/1017228/18878/i/950/depositphotos_188781580-stock-photo-handsome-cheerful-young-man-standing.jpg"}
        ...
        <div className="flex flex-row items-center justify-end mr-2 basis-1/3">
          ...
          <p className="mr-2">{data?.rating.toFixed(2)}</p>
        </div>
      </div>
    </div>
  );
