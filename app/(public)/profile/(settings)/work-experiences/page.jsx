    ...
    <div className="grid gap-6">
      <div className="grid gap-2">
        <div className="flex items-center justify-between bg-primary" style={{ borderTopLeftRadius: "0.5rem", borderTopRightRadius: "0.5rem" }}>
          ...
          <p className="relative px-3 py-3 text-white cursor-pointer select-none hover:text-white/40 group" style={{ borderLeft: "2px solid white" }} onClick={handleOnAdd}>
            +<span className="absolute p-2 text-xs text-white scale-0 bg-gray-800 rounded top-10 group-hover:scale-100">Tambah Data</span>
          </p>
        </div>
        ...
        {workExperiences.map((v, i) => (
          <div className="flex items-center" key={i}>
            <div className="grid w-full gap-2">
              ...
              <TextArea id="description" placeholder="Keterangan" onChange={(v) => handleOnChange(v, i, "description")} value={workExperiences[i]["description"]} maxlength={500} />
            </div>
            <div className="flex items-center justify-center" style={{ minWidth: "63.91px" }}>
              <div className="relative p-2 cursor-pointer select-none group" onClick={() => handleOnDelete(i)}>
                <TrashIcon width={24} height={24} color="gray" />
                <span className="absolute p-2 text-xs text-white scale-0 bg-gray-800 rounded top-10 group-hover:scale-100">Hapus Data</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="flex flex-row justify-center mt-5 ">
        ...
        <Button type="button" onClick={handleOnSave} label="Simpan" className="px-8 mx-2 w-fit" disabled={isLoading} />
      </div>
    </div>
  );
}
