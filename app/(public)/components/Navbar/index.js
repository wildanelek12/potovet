    ...
    <nav className={"py-6 px-16 shadow-md flex items-center bg-white text-white"}>
      ...
      <div className="flex-row flex-1 space-x-12">
        <div className="relative inline-flex items-center hover:text-primary">
          <div className={`text-base font-medium text-[#6E6F70] flex items-center hover:text-primary `} onClick={handleFeaturesClick}>
            Features
          ...
          {isDropdownOpen && (
            <div
              className="absolute top-5 mt-2 p-5 gap-y-4 gap-x-4  bg-white divide-y grid grid-cols-2 text-black divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none "
              style={{ width: "500px" }}
            >
              <div className="flex flex-col">
                <span className="font-semibold">Portfolio Album</span>
                <span className="font-normal text-sm">
                  Tampilan portofolio secara sistematis dengan konsep gallery untuk menyajikan portfolio yang telah dibuat. Konsep ini akan diatur berdasarkan filter dan sorting waktu.
                </span>
              </div>
              <div className="flex flex-col">
                <span className="font-semibold">Manage a Portfolio </span>
                <span className="font-normal text-sm">
                  Menampilkan visual data yang terbentuk dalam pembuatan portofolio secara real time, mengatur editing portofolio, hak akses portofolio, dan penyampaian resource projek.
                </span>
              </div>
              <div className="flex flex-col">
                <span className="font-semibold">Manage User </span>
                <span className="font-normal text-sm">User dapat mengelola data yang berkaitan dengan informasi diri untuk menunjang pengembangan user.</span>
              </div>
              <div className="flex flex-col">
                <span className="font-semibold">Create a Portfolio</span>
                <span className="font-normal text-sm">
                  membuat portfolio berbentuk dokumen dan online. User dapat membuat portofolio dengan konten yang sistematis dan dukungan template deskripsi.
                </span>
              </div>
            </div>
          )}
                  ...
                  <Menu.Items className="absolute right-0 w-56 mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div className="px-1 py-1">
                      <Link href={"/clientzone"}>
                        <Menu.Item>
                          {({ active }) => <button className={`${active ? "bg-primary text-white" : "text-gray-900"} group flex w-full items-center rounded-md px-2 py-2 text-sm`}>Portfolio</button>}
                        </Menu.Item>
                      </Link>
                    </div>
                    <div className="px-1 py-1">
                    ...
                    <div className="px-1 py-1">
                      <Link href={"/profile"}>
                        <Menu.Item>
                          {({ active }) => <button className={`${active ? "bg-primary text-white" : "text-gray-900"} group flex w-full items-center rounded-md px-2 py-2 text-sm`}>Settings</button>}
                        </Menu.Item>
                      </Link>
                    </div>
