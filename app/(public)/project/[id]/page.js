          ...
          <>
            <Modal isOpen={isOpenShare} onClose={closeModal}>
              ...
              <p>
                ...
                <span>
                  {" "}
                  <button onClick={() => closeModalShare()} className="py-1 px-3 text-sm font-semibold text-white rounded-full bg-slate-400 flex mt-2 items-center">
                    <MdCopyAll />
                    Copy to clipboard
                  </button>
                </span>{" "}
          ...
          <div className="fixed flex flex-col bottom-20 right-10">
            ...
            {isShowButton ? (
              <>
                <Link href={"/clientzone/project-list/1"}>
                  <button className="p-6 mb-2 text-xl font-semibold text-white rounded-full bg-primary group relative">
                    <BsShareFill />
                    <span class="absolute  top-10 scale-0 rounded bg-gray-800 p-2 text-xs text-white group-hover:scale-100">Share</span>
                  </button>
                </Link>
                <button className="p-6 mb-2 text-xl font-semibold text-white rounded-full bg-primary group relative">
                  <BsPrinter />
                  <span class="absolute  top-10 scale-0 rounded bg-gray-800 p-2 text-xs text-white group-hover:scale-100">Print</span>
                </button>
                {/* <Link href={"/clientzone/project-list/1"}>
              <button className="p-6 mb-2 text-xl font-semibold text-white rounded-full bg-primary">
                ...
                <Link className="p-6 mb-2 text-xl font-semibold text-white rounded-full bg-primary group relative" href={"mailto:wildan.romiza@gmail.com?subject=SendMail&body=Description"}>
                  <MdOutlineEmail />
                  <span class="absolute  top-10 scale-0 rounded bg-gray-800 p-2 text-xs text-white group-hover:scale-100">Message</span>
                </Link>
                <CommentsDialog setModalComments={setIsShowComments} showModalComments={isShowComments} />
              </>
            ) : (
              <></>
            )}
