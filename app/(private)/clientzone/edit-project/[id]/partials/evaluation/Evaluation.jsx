...
export default function Evaluation({ data }) {
	const { status, backwardStep } = useEditProjectStore()
	const { formData, setFormData, setInitialFormData, getIsFilledFormData } = useEvaluationStore()
	...
	return (
		<form className="flex flex-col w-full gap-6" {...formProps}>
			...
			<div className="flex flex-row justify-between">
				...
				<Button
					label={status === 'PUBLISH' ? 'Simpan' : 'Publish'}
					type="submit"
